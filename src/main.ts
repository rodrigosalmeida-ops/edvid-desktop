...
async function ensurePreviewProxy(
  ...
  const job = (async (): Promise<string | null> => {
    ...
    const trunk = currentName.replace(new RegExp(`_${PROXY_VERSAO}\\.mp4$`, 'u'), '');
    const siblings = await readdir(folder).catch(() => [] as string[]);
    await Promise.all(
      siblings
        .filter((sibling) => sibling !== currentName && (sibling.startsWith(`${trunk}_`) || sibling === `${trunk}.mp4`))
        .map((sibling) => rm(path.join(folder, sibling), { force: true }).catch(() => {})),
    );
    ...
    try {
      ...
      await filaTrabalhoPesado.adicionar(() => new Promise<void>((resolve, reject) => {
        ...
        child.stderr.setEncoding('utf8');
        child.stderr.on('data', (chunk: string) => {
          if (errorText.length < 4000) errorText += chunk;
        });
