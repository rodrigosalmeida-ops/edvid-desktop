...
async function ensurePreviewProxy(
  ...
  const job = (async (): Promise<string | null> => {
    ...
    const currentName = path.basename(destination);
    const trunk = currentName.replace(new RegExp(`_${PROXY_VERSAO}\\.mp4import { app, autoUpdater, BrowserWindow, dialog, ipcMain, net, protocol, session, shell } from 'electron';
import started from 'electron-squirrel-startup';
import { spawn } from 'node:child_process';
import { createReadStream, createWriteStream } from 'node:fs';
import { createHash, randomUUID } from 'node:crypto';
...
, 'u'), '');
    ...
    await Promise.all(siblings
      .filter((name) => name !== currentName && (name.startsWith(`${trunk}_`) || name === `${trunk}.mp4`))
      .map((name) => rm(path.join(folder, name), { force: true }).catch(() => {})));
