...
// workspace-write (escrita so no projeto e nos caches do EDIT AI) e a rede
// permanece negada.
...
- J-CUT NAO E TAREFA SUA: o EDIT AI aplica e reaplica o J-cut sozinho, fora do sandbox, remontando apenas o audio do corte (o video fica intacto). Nunca antecipe audio por conta propria, nao escreva o campo jcut_timeline no EDL e nao apague edit/jcut.json nem arquivos *-sem-jcut-tmp* — sao o estado e o backup dessa aplicacao.
...
- Registro sem kind e sem codigo proprio nao fica invisivel (o EDIT AI deduz um tipo pelo rotulo), mas o resultado fica pobre: um nome interno vira o texto que o espectador le. Decida voce.
...
export class CodexAppServer {
  ...
  private async writeSandboxConfig(): Promise<void> {
    ...
    const config = [
      ...
      ...(engine ? [`model_provider = ${JSON.stringify(`editai-${engine.providerId}`)}`] : []),
      ...
      ...(engine
        ? [
          `[model_providers.editai-${engine.providerId}]`,
  ...
  private async startInternal(): Promise<void> {
    ...
    // A CLI pode encerrar entre o spawn e o primeiro RPC (por exemplo, em um
    // healthcheck). Sem tratar o erro do pipe, um EPIPE derruba todo o Electron.
  ...
  async runUtilityTurn(
    ...
    const started = await this.request<ThreadStartResponse>('thread/start', {
      ...
      approvalPolicy: CODEX_APPROVAL_POLICY,
      sandbox: CODEX_SANDBOX,
      serviceName: 'editai_desktop_imagens',
      model: this.engine?.model ?? CODEX_CHAT_MODEL,
    });
